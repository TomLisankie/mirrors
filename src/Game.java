...
public class Game extends Canvas {
	...
	public void gameLoop() {
		...
		while (gameRunning == true) {
			...
			//paints each layer
			for(int i = 0; i < lm.getNumberOfLayers(); i++){
				
				for(int k = 0; k < lm.getLayer(i).getObjectsOnLayer().size(); k++){
					
					 BufferedImage img = lm.getLayer(i).getObjectsOnLayer().get(k).getImage();
